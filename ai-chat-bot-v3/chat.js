...
async function processConversation(userId, message, lang, userType, langResult) {
    ...
    // 4. Captain Flow (early branch) - with database verification
    if (userType === 'captain') {
        // Double-check captain access
        ...
        if (!captainCheck.verified) {
            ...
            userType = 'customer';
            logSecurityEvent('captain_access_denied_in_flow', {
                userId,
                reason: captainCheck.reason
            });
        } else {
            // Verified captain - use captain flow
...
/**
 * Handle captain-specific flows
 */
async function handleCaptainFlow(userId, message, lang, convState = null) {
    ...
    if (useHybridClassifier) {
        try {
            ...
            logger.warn('Captain classifier failed, using regex', { error: error.message });
            classification = classifyIntent(message, 'captain');
        }
    } else {
        classification = classifyIntent(message, 'captain');
    }
    ...
    // Captain earnings
    if (/\b(earnings|كسبت|أرباح|arba7|kasabt)\b/i.test(message)) {
        return {
            message: lang === 'ar'
                ? '💰 أرباح اليوم: 1,250 ج.م\n📊 عدد الرحلات: 8'
                : '💰 Today\'s Earnings: 1,250 EGP\nTrips: 8',
            action: 'show_earnings',
            data: { earnings: 1250, trips: 8 },
            quick_replies: lang === 'ar'
                ? ['ملخص الأسبوع', 'سجل المدفوعات']
                : ['Weekly Summary', 'Payment History'],
            userType: 'captain',
            language: lang
        };
    }
    ...
    // Captain next pickup
    if (/\b(next|pickup|طلب|جايلي|feen)\b/i.test(message)) {
        return {
            message: lang === 'ar'
                ? '📍 الطلب القادم: ماكدونالدز، عباس العقاد\n📏 المسافة: 2.5 كم'
                : '📍 Next Pickup: McDonald\'s, Abbas El Akkad\nDistance: 2.5km',
            action: 'show_next_pickup',
            data: { lat: 30.0444, lng: 31.2357 },
            quick_replies: lang === 'ar'
                ? ['قبول', 'رفض']
                : ['Accept', 'Reject'],
            userType: 'captain',
    ...
    return {
        message: lang === 'ar'
            ? '👋 مرحباً كابتن! إزاي أقدر أساعدك؟'
            : '👋 Hello Captain! How can I help you?',
        action: ACTION_TYPES.NONE,
        quick_replies: lang === 'ar'
            ? ['أرباحي', 'الطلب القادم', 'إبلاغ عن مشكلة']
            : ['My Earnings', 'Next Pickup', 'Report Issue'],
        userType: 'captain',
        language: lang
    };
}
