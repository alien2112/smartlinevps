 ...
 * Defines which versions can migrate to which versions
 */
...
class StateGuard {
  ...
  async checkStateVersion(userId, currentState) {
    ...
    if (currentState.version < CURRENT_STATE_VERSION) {
      ...
      const compatibility = STATE_COMPATIBILITY[currentState.version];
      if (!compatibility || !compatibility.compatible.includes(CURRENT_STATE_VERSION)) {
        // Not compatible - check if breaking changes
        const hasBreaking = compatibility?.breaking?.length > 0;
        return {
          valid: false,
          action: hasBreaking ? 'RESET' : 'MIGRATE',
          reason: hasBreaking ? 'VERSION_BREAKING' : 'VERSION_INCOMPATIBLE',
          from: currentState.version,
          to: CURRENT_STATE_VERSION,
...
module.exports.STATE_COMPATIBILITY = STATE_COMPATIBILITY;