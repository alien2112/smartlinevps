...
class Helpers
{
    ...
    public static function set_data_format($data)
    {
         ...
         if (!isset($data['colors']) || $data['colors'] === null) {
                return null;  // or you can return an empty array or some default value
            }
    ...
    public static function get_price_range_with_discount($product)
{
    ...
    // Initialize with unit price in case there are no variations
    $lowest_price = $product->unit_price;
    $highest_price = $product->unit_price;

    // Check variations to find lowest and highest prices
    if (!empty($variations)) {
        foreach ($variations as $variation) {
            $variation_price = $variation->discounted_price ?? $variation->price;
            if ($lowest_price > $variation_price) {
                $lowest_price = round($variation_price, 2);
            }
            if ($highest_price < $variation_price) {
                $highest_price = round($variation_price, 2);
            }
    ...
    // Calculate the product-level discount on base price if any
    if ($product->discount > 0 ) {
        $discounted_lowest_price = Helpers::currency_converter($lowest_price - Helpers::get_product_discount($product, $lowest_price));
        $discounted_highest_price = Helpers::currency_converter($highest_price - Helpers::get_product_discount($product, $highest_price));

        // Return appropriate price display based on theme
        if ($discounted_lowest_price == $discounted_highest_price) {
            if ($discounted_lowest_price == self::currency_converter($lowest_price)) {
                return $discounted_lowest_price;
            } else {
                return theme_root_path() === "default" 
                    ? $discounted_lowest_price." <del class='align-middle fs-16 text-muted'>".self::currency_converter($lowest_price)."</del> " 
                    : $discounted_lowest_price." <del>".self::currency_converter($lowest_price)."</del> ";
            }
        }

        return theme_root_path() === "default" 
            ? "<span>".$discounted_lowest_price."</span>"." <del class='align-middle fs-16 text-muted'>".self::currency_converter($lowest_price)."</del> ". ' - ' ."<span>".$discounted_highest_price."</span>"." <del class='align-middle fs-16 text-muted'>".self::currency_converter($highest_price)."</del> " 
            : $discounted_lowest_price." <del>".self::currency_converter($lowest_price)."</del> ". ' - ' .$discounted_highest_price." <del>".self::currency_converter($highest_price)."</del> ";
    } else {
        // Return appropriate price display without discount
        if (!empty($variations)) {
            return theme_root_path() === "default" 
                ? "<span>".self::currency_converter($lowest_price)."</span>".' - ' ."<span>".self::currency_converter($highest_price)."</span>" 
                : self::currency_converter($lowest_price). ' - ' .self::currency_converter($highest_price);
        } else {
            return theme_root_path() === "default" 
                ? "<span>".self::currency_converter($lowest_price)."</span> " 
                : self::currency_converter($lowest_price);
        }
    }
}
    ...
    public static function module_permission_check($mod_name)
    {
        ...
        if (auth('admin')->user()->admin_role_id == 1|| auth('seller')->user()->admin_role_id == 1 ) {
            return true;
        }
        return false;
    }
    public static function module_seller_permission_check($mod_name)
    {
        $seller_role = auth('seller')->user()->role;
        $permission = $seller_role->module_access;
        if (isset($permission) && $seller_role->status == 1 && in_array($mod_name, (array)json_decode($permission)) == true) {
            return true;
        }

        if (auth('seller')->user()->seller_role_id == 1) {
            return true;
        }
        return false;
    }
    ...
    public static function send_push_notif_to_topic($data, $topic = 'sixvalley')
{
    ...
    $image = asset('storage/app/public/notification') . '/' . $data['image'];
    $productLink = route('product', ['slug' => $data->product_slug]); // Adjust the route as needed
    $type = "notification";
// dd($productLink);
    $postdata = '{
        "to" : "/topics/' . $topic . '",
    ...
    // Use your preferred method to send HTTP request with $url, $header, and $postdata
