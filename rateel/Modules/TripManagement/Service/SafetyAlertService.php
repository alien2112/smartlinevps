...
class SafetyAlertService extends BaseService implements Interface\SafetyAlertServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        ...
        $formattedAddress = $responseData->data->address ?? ($responseData->data->formatted_address ?? 'N/A');
        
        $attributes = [];
        $attributes['trip_request_id'] = $data['trip_request_id'];
        $attributes['sent_by'] = auth('api')->user()?->id;
        $attributes['alert_location'] = $formattedAddress;
    ...
    public function updatedBy(array $criteria = [], array $whereInCriteria = [], array $data = [], bool $withTrashed = false): ?Model
    {
        ...
        // GeoLink Reverse Geocode API
        $response = Http::get(MAP_API_BASE_URI . '/api/v2/reverse_geocode', [
            'latitude' => $trip?->driver?->lastLocations?->latitude,
            'longitude' => $trip?->driver?->lastLocations?->longitude,
            'key' => $mapKey
        ]);
        ...
        $attributes = [];
        $attributes['resolved_location'] = $formattedAddress;
