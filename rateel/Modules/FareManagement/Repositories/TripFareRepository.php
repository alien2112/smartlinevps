...
class TripFareRepository implements TripFareInterface
{
    ...
    public function store(array $attributes): Model
    {
        ...
        $defaultTripFare->minimum_distance_threshold = $attributes['minimum_distance_threshold'] ?? 0;
        $defaultTripFare->fixed_price_below_threshold = $attributes['fixed_price_below_threshold'] ?? 0;
        ...
        foreach (($attributes['vehicleCategories']) as $vehicleCategories) {
            ...
            if (array_key_exists('vehicle_category_' . $vehicleCategories->id, $attributes)) {
                ...
                if ($attributes['category_wise_different_fare'] == 0) {
                    ...
                    $tripFare->minimum_distance_threshold = $attributes['minimum_distance_threshold'] ?? 0;
                    $tripFare->fixed_price_below_threshold = $attributes['fixed_price_below_threshold'] ?? 0;
                } else {
                    ...
                    $tripFare->minimum_distance_threshold = $attributes['minimum_distance_threshold' . $vehicleCategories->id] ?? 0;
                    $tripFare->fixed_price_below_threshold = $attributes['fixed_price_below_threshold' . $vehicleCategories->id] ?? 0;
                }
                $tripFare->save();
                $last_query = $tripFare;
            }
