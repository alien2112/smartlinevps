                ...
                <form action="{{route('admin.update',[$notification['id']])}}" method="post"
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="form-group mb-4">
                                <label class="input-label" for="send_to">{{ translate('sent_to') }}</label>
                                <select name="sent_to" id="send_to" class="form-control" required>
                                    <option value="" disabled selected>{{ translate('choose') }}</option>
                                    <option value="customer" {{ $notification['sent_to'] == 'customer' ? 'selected' : '' }}>
                                        {{ translate('customer') }}
                                    </option>
                                    <option value="captin" {{ $notification['sent_to'] == 'captin' ? 'selected' : '' }}>
                                        {{ translate('captin') }}
                                    </option>
                                </select>
                            </div>
