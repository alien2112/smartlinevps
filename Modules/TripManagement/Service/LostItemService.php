...
class LostItemService extends BaseService implements LostItemServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        ...
        try {
            ...
            // Publish event for realtime updates (post-commit to avoid phantom events)
    ...
    {
        try {
            // Load relations so sockets receive a complete payload without another API call
            ...
            $payload = json_encode([
                'event' => $event,
                // Flat fields preserved for backward compatibility with existing socket handlers
                'id' => $lostItem->id,
                ...
                // Full resource for richer client updates
                'lost_item' => $resource,
            ]);
